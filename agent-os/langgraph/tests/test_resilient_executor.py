...
def test_primary_success_never_touches_fallback():
    ...
    result = run_executor_resilient(_pass_gate_result(), client=primary, fallback_client="SHOULD_NOT_BE_USED")

    assert result["status"] == "EXECUTED"
    assert result["output"] == "Bio mise à jour."


def test_primary_quota_failure_falls_back_to_second_agent():
...
def test_both_providers_failing_raises_executor_error():
    ...
    with pytest.raises(ExecutorError):
        run_executor_resilient(_pass_gate_result(), client=primary, fallback_client=fallback)
...
def test_gate_not_passed_never_falls_back_around_governance():
    ...
    the fallback must never execute a task the gate deliberately blocked."""
    stopped_gate = _pass_gate_result(writeback={"status": "STOP"})

    with pytest.raises(ExecutorError):
        run_executor_resilient(stopped_gate, client="SHOULD_NOT_BE_CALLED", fallback_client="SHOULD_NOT_BE_CALLED")
